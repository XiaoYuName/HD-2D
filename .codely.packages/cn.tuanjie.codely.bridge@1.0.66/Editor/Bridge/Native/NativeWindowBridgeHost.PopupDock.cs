...
namespace UnityTcp.Editor.Native
{
    ...
    internal static partial class NativeWindowBridgeHost
    {
        ...
        private static HashSet<int> s_CompositeKnownWindowIds = new HashSet<int>();
        ...
        private static void TrackUntrackedCompositePanes()
        {
            ...
            foreach (var slot in s_CompositeSlots.Values)
                if (slot?.Window != null)
                    trackedIds.Add(slot.Window.GetInstanceID());
            ...
            {
                if (w == null || trackedIds.Contains(w.GetInstanceID())) continue;
                ...
                w.Repaint();
                anyAdded = true;
                CodelyLogger.Log($"[NWB-Composite] Auto-tracked native tab: {typeName} (id={w.GetInstanceID()})");
            }
        ...
        private static void InitCompositeWindowTracking()
        {
            ...
            {
                if (w != null)
                    s_CompositeKnownWindowIds.Add(w.GetInstanceID());
            }
        ...
        internal static bool InterceptAndDockNewPopupWindows()
        {
            ...
            var compositeInstanceIds = new HashSet<int>();
            foreach (var slot in s_CompositeSlots.Values)
            {
                if (slot?.Window != null)
                    compositeInstanceIds.Add(slot.Window.GetInstanceID());
            }
            ...
            foreach (var w in newWindows)
            {
                ...
                if (compositeInstanceIds.Contains(w.GetInstanceID()))
                {
