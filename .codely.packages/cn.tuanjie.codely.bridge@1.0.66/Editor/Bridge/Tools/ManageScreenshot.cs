...
namespace UnityTcp.Editor.Tools
{
    ...
    public static class ManageScreenshot
    {
        ...
        public static object HandleCommand(JObject @params)
        {
            ...
            if (ParseGifParams(@params) != null)
                return Response.Success("as_gif requires multi-frame rendering; use HandleCommandCoroutine.");
        ...
        {
            return Mathf.Clamp(AsFloat(p?["scale"]) ?? 0.5f, 0.25f, 1.0f);
        }
            ...
            Action<object> setResult)
        {
            ...
            for (int i = 0; i < gif.FrameCount; i++)
            {
                if (i > 0)
                {
                    // Pace to the target fps (advances editor/simulation between captures).
