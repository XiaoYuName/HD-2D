...
public class UIAutoBindGenerator : MonoBehaviour
{
    ...
    internal static void RefreshItems(UIAutoBindGenerator generator, bool fillEmptyFieldNames)
    {
        ...
        foreach (UIAutoBindItem item in generator.BindItems)
        {
            ...
            if (string.IsNullOrEmpty(item.ComponentTypeName))
            {
                item.ComponentTypeName = GetTypeStorageName(PickDefaultType(item.Target));
            }
    ...
    private static List<GeneratedBindItem> BuildGeneratedItems(UIAutoBindGenerator generator)
    {
        ...
        foreach (UIAutoBindItem item in generator.BindItems)
        {
            ...
            if (type != typeof(GameObject) && item.Target.GetComponent(type) == null)
            {
            ...
            generatedItems.Add(new GeneratedBindItem(fieldName, type, relativePath));
        }
    ...
    private static string BuildAutoBindScriptContent(string className, List<GeneratedBindItem> items)
    {
        ...
        {
            builder.AppendLine($"    private {GetTypeNameForCode(item.Type)} {item.FieldName};");
        }
    ...
    internal static Type PickDefaultType(GameObject targetObject)
    {
        ...
        List<Type> types = GetBindableTypes(targetObject);
    ...
    internal static List<Type> GetBindableTypes(GameObject targetObject)
    {
    ...
    private readonly struct GeneratedBindItem
    {
        ...
        public GeneratedBindItem(string fieldName, Type type, string path)
        {
            FieldName = fieldName;
            Type = type;
            Path = path;
...
public class UIAutoBindItem
{
    ...
    {
        ComponentTypeName = UIAutoBindGenerator.GetTypeStorageName(UIAutoBindGenerator.PickDefaultType(Target));
    ...
    private IEnumerable<ValueDropdownItem<string>> GetComponentTypeOptions()
    {
        ...
        foreach (Type type in UIAutoBindGenerator.GetBindableTypes(Target))
        {
