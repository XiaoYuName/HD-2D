...
namespace UnityMcp
{
    ...
    static class BridgeRouter
    {
        ...
        static BridgeRouter()
        {
            foreach (string toolName in SourceCodeBridge.ToolNames)
                Handlers.Add(toolName, SourceCodeBridge.Dispatch);
        }
        ...
        public static bool CanRunInBackground(string requestJson)
        {
            try
            {
                string action = (string)JObject.Parse(requestJson ?? string.Empty)["action"];
                return !string.IsNullOrEmpty(action) && SourceCodeBridge.Handles(action);
            }
            catch
            {
                return false;
        ...
        public static string ProjectPath() => ProjectRoot;
    }
}
