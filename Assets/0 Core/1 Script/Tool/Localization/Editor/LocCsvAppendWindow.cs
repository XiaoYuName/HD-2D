...
public class LocCsvAppendWindow : EditorWindow
{
    ...
    LocCsvEditor.ParseResult Parse()
        => csvField.value is TextAsset ta ? LocCsvEditor.Parse(ta.text) : default;
    ...
    void RebuildLangFields()
    {
        ...
        LocCsvEditor.ParseResult p = Parse();
        if(!p.ok)
        { status.text = "✗ " + p.message; preview.text = ""; appendBtn?.SetEnabled(false); return; }

        foreach(LocCsvEditor.Column col in p.columns)
        {
    ...
    void Refresh()
    {
        ...
        preview.text = "预览：" + LocCsvEditor.BuildRow(p.columns, key, Values());
    }

    void Append()
    {
        string path = AssetPath;
        if(!LocCsvEditor.AppendEntry(path, keyField.value, Values(), out string msg))
    ...
    string TryImport(string assetPath)
    {
        ...
        LocCsvMerger.Result r = LocCsvMerger.Merge(File.ReadAllText(Path.GetFullPath(assetPath)), col, overwrite: true);
        return r.ok
