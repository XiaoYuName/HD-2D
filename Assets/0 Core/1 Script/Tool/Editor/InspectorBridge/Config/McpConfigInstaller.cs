...
namespace UnityMcp
{
    ...
    static class McpConfigInstaller
    {
        public const string McpScriptAssetPath =
            "Assets/0 Core/1 Script/Tool/Editor/InspectorBridge/McpServer/unity-prefab-mcp.ps1";
        ...
        const string CodexBeginMarker = "# BEGIN InspectorBridge Codex MCP (generated)";
        const string CodexEndMarker = "# END InspectorBridge Codex MCP (generated)";
        ...
        public static void InstallOrUpdateOnStartup(int port)
        {
            ...
            {
                Debug.LogWarning($"[InspectorBridge] 启动时同步 MCP 配置失败：{e.Message}");
            }
        }
        ...
        static void InstallOrUpdateCodex(int port)
        {
            ...
            if (!File.Exists(scriptPath))
                throw new FileNotFoundException("找不到 Inspector Bridge MCP 启动脚本。", scriptPath);
            ...
            Debug.Log($"[InspectorBridge] Codex MCP 配置已写入：{CodexConfigRelativePath}");
        }
