...
namespace UnityMcp
{
    ...
    /// 对象引用支持三种写法，且与 get_component_fields 读出来的格式一致（可原样回填）：
    ...
    static class SerializedValueWriter
    {
        ...
        public static string Write(SerializedProperty property, Component component, Transform rootTf, string value)
        {
            ...
            switch (property.propertyType)
            {
                ...
                case SerializedPropertyType.ObjectReference:
                    return WriteObjectReference(property, component, rootTf, text);
                default:
        ...
        static string WriteObjectReference(SerializedProperty property, Component component, Transform rootTf, string text)
        {
            ...
            {
                resolved = ResolveNode(rootTf, nodeSpec, expectedType, out error);
            }
            else
            {
                return "对象引用字段的 value 必须是 null、asset:<Assets 路径>[#子资产名|@objectId[#componentIndex]] " +
                    "或 object:<objectId>[#componentIndex]";
            }
        ...
        static UnityEngine.Object ResolveAsset(string spec, Type expectedType, out string error)
        {
            ...
            if (at >= 0)
            {
                ...
                return ResolveNode(prefab.transform, spec.Substring(at + 1), expectedType, out error);
            }
        ...
        /// <summary>解析 objectId[#componentIndex][:类型名]；末尾的类型名只是可读后缀，忽略。</summary>
        static UnityEngine.Object ResolveNode(Transform rootTf, string spec, Type expectedType, out string error)
        {
            ...
            if (!PrefabAddress.TryGetObject(rootTf, idPart, out Transform nodeTf, out error))
                return null;
