...
namespace UnityMcp
{
    ...
    static class ScreenshotTool
    {
        public static string Capture(ScreenshotRequest command)
        {
            ...
            if (!custom && !gameView && !string.Equals(mode, "focusedWindow", StringComparison.OrdinalIgnoreCase))
                return BridgeJson.Fail("captureTarget 只能是 focusedWindow、custom 或 gameView");
