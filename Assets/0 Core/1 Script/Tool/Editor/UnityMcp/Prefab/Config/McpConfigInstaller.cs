...
namespace UnityMcp
{
    ...
    static class McpConfigInstaller
    {
        ...
        {
            string scriptPath = Path.Combine(ProjectRoot, McpScriptPath.Replace('/', Path.DirectorySeparatorChar));
        ...
        static string BuildCodexBlock(int port, string newline)
        {
            string command = "$projectRoot = (& git rev-parse --show-toplevel).Trim(); " +
                $"& (Join-Path $projectRoot '{McpScriptPath}') -ProjectPath $projectRoot -Port {port}";
