    ...
    static class ToolCatalog
    {
        ...
        static void Export()
        {
            ...
            try
            {
                ...
                if (missing.Length > 0)
                    Debug.LogError($"[UnityMcp] 工具表里有 BridgeRouter 未注册的工具: {missing}");
                if (handled.Count > 0)
                    Debug.LogError($"[UnityMcp] BridgeRouter 注册了工具表里没有的 action: {string.Join(", ", handled)}");
            ...
            {
                Debug.LogWarning($"[UnityMcp] 导出 MCP 工具表失败（Unity 关闭时客户端将拿不到工具列表）: {e.Message}");
            }
        }

        public static JArray Build()
        {
            JArray tools = new(new List<JObject>
            {
            ...
            Tool("get_unity_compile_status",
                "Read current compilation state and the persisted errors/warnings from the latest script compilation, including after an assembly reload. compileStatus.resultStale=true means the result predates your refresh_unity_assets call, so errorCount is not about your change yet - pass waitSeconds to wait it out instead of polling yourself. A field you just added to a script does not exist for edit_prefab until compilation finishes.",
                new JObject
                {
                    ["excludeMessages"] = Bool(false, "Return counts only."),
                    ["maxResults"] = Int(1, 200, 50, "Maximum errors/warnings to return."),
                    ["waitSeconds"] = Number("Wait until the result is fresh (not compiling, not stale) before returning, up to this many seconds. Use after refresh_unity_assets, e.g. 180. Survives the domain reload that compilation triggers.", 0, 600),
                }),

            Tool("capture_unity_screenshot",
                "Capture a Unity view as a downscaled JPEG. sceneView renders the Scene View camera offscreen - no window focus, no PlayMode, works while the Editor is in the background - so it is the way to verify a layout you just edited: open_prefab_stage, edit, then capture with frameContent=true. gameView needs PlayMode and uses screen coordinates shared with simulate_playmode_input; annotateUi draws numbered bounds around interactive uGUI elements and returns their paths and coordinates; elementsOnly skips image capture.",
                new JObject
                {
                    ["captureTarget"] = Enum(ScreenshotTool.SupportedTargets, ScreenshotTool.SupportedTargets[0]),
                    ["frameContent"] = Bool(false, "sceneView only: aim the view at the Prefab currently in edit mode (or the scene selection) before rendering."),
                    ...
                    ["widthPixels"] = Int(1, description: "custom: rectangle width. sceneView: render width, defaults to the window width."),
                    ["heightPixels"] = Int(1, description: "custom: rectangle height. sceneView: render height, defaults to the window height."),
            ...
            TargetTool("edit_prefab",
                "Run an ordered transaction. Writes by default; dryRun=true previews and returns planId+revision, then a second call with only planId commits without resending operations. expectedRevision prevents lost updates; idempotencyKey makes retries safe. Default responseMode=summary keeps only transaction state and created/component references. createIfMissing can build a new prefabAsset. \"$n\" references an earlier op's node.",
                new JObject
                {
                    ["dryRun"] = Bool(false, "Omit to write: the batch backs up once, runs, and saves once. true previews the whole batch in memory without saving (prefabAsset only; live prefabStage/openScene targets reject it)."),
                    ["planId"] = Str("Commit a prior dryRun by sending only this planId; omit operations. Commit is rejected if the Prefab revision changed."),
                    ["expectedRevision"] = Str("Optional optimistic-concurrency precondition. Use the revision returned by dryRun; a mismatch rejects the edit before mutation."),
                    ["idempotencyKey"] = Str("Optional retry key (1-200 characters). Same key+request replays the saved result; reusing it for different input is rejected."),
                    ["responseMode"] = Enum(new[] { "summary", "changed", "full" }, "summary",
                        "summary returns transaction state plus created/component references; changed returns all op results without hierarchyPath; full preserves the legacy detailed response."),
                    ["createIfMissing"] = Object(new JObject
                    {
                        ["rootName"] = Str("Optional root name; defaults to prefabPath's file name."),
                        ["transformType"] = Enum(EditTarget.SupportedRootTransformTypes,
                            EditTarget.SupportedRootTransformTypes[0],
                            "Root component: Transform (default) or RectTransform."),
                    }),
                    ["operations"] = ArrayOf(Object(new JObject
                    {
                        ...
                        ["componentType"] = Str("addComponent/ensureComponent (required), or setValue/setValues/removeComponent unique-type selector. Short or full name; ambiguity is rejected."),
                        ["componentIndex"] = Int(description: "Fallback selector for setValue/setValues/removeComponent. Prefer componentRef or unique componentType because indices shift."),
                        ["componentRef"] = Str("addComponent/ensureComponent: optional batch alias to declare. Component operations: alias, $n for a prior component-producing op, or object:<objectId>#<componentIndex> returned by a prior response. Takes precedence over componentType/index."),
            ...
            foreach (JToken definition in SourceCodeBridge.GetToolDefinitions())
        ...
        {
            properties["prefabPath"] = Str("Assets/.../*.prefab path. Required for prefabAsset; edit_prefab may target a missing path only with createIfMissing.");
            properties["targetMode"] = Enum(EditTarget.SupportedModes, EditTarget.SupportedModes[0],
                "Edit target. prefabAsset (default) reads/writes the on-disk prefab and supports dryRun; prefabStage uses the currently open Prefab stage; openScene uses sceneRootName in the active scene. Live stage/scene targets mutate real objects and only mark the scene dirty (save with Ctrl+S), with the whole batch registered as one Undo step.");
