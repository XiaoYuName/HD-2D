...
namespace UnityMcp
{
    ...
    public static class BridgeServer
    {
        ...
        public static void Start()
        {
            ...
            for (int offset = portFloorOffset; offset <= portFloorOffset + PortScanRange; offset++)
            {
                ...
                if (TryBind(port, out string bindError))
                {
                    ...
                    listenerThread = new Thread(ListenLoop) { IsBackground = true, Name = "UnityMcp.PrefabBridge" };
                    listenerThread.Start();
                    EditorApplication.update += Pump;
                    EditorApplication.update += HealthWatch;
                    QueueSelfProbe(port, instanceToken);
                    if (offset > 0)
                        Debug.LogWarning($"[UnityMcp] 端口 {settings.Port} 被占用，已改用 {port}" +
            ...
            if (startRetryCount++ < StartRetryLimit)
                ...
                Debug.LogWarning($"[UnityMcp] 启动失败：{settings.Port}~{settings.Port + PortScanRange} 全部无法绑定" +
        ...
        static void WriteActivePortFile(int port)
        {
            ...
            {
                Debug.LogWarning($"[UnityMcp] 写入端口文件失败，客户端需要手动指定 -Port {port}: {e.Message}");
            }
        }
        ...
        static void HealthWatch()
        {
            ...
            if (healthShiftCount++ >= HealthShiftLimit)
            {
                ...
                Debug.LogWarning("[UnityMcp] " + lastStartError);
                return;
            }

            portFloorOffset = Math.Max(portFloorOffset, contestedPort - configuredPort + 1);
            Debug.LogWarning($"[UnityMcp] 端口 {contestedPort} 上有另一个监听实例（多半是域重载后残留的）在抢连接，" +
        ...
        static void HandleClient(Socket client)
        {
            try
            {
                ...
                WriteResponse(client, BridgeRouter.CanRunInBackground(body)
                    ? BridgeRouter.Dispatch(body)
                    : Dispatch(body));
            }
            catch { /* 客户端可能已断开 */ }
            finally
