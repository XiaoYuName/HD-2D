// UI 暗角叠加层着色器
// 用于一张铺满拍摄区域、置于最上层的 RawImage：中心透明、边缘渐变为暗角颜色，
// 通过正常透明混合压暗其后的所有内容（背景 + 角色），实现全局统一的暗角光影。
// 兼容 Built-in 与 URP（UGUI Overlay 渲染与渲染管线无关）。
Shader "UI/UIVignetteOverlay"
{
    Properties
    {
