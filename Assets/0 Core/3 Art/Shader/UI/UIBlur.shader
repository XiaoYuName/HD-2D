// UI 图片高斯模糊着色器
// 用于 UGUI Image：通过 _BlurSize 控制模糊强度（0 = 完全清晰）。
...
Shader "UI/UIBlur"
{
    ...
    SubShader
    {
        ...
        CGPROGRAM
            ...
            // 圆盘散景模糊：黄金角螺旋在圆盘内均匀采样模拟相机失焦圆斑。
            // 每像素随机旋转采样图案，把稀疏采样的环状条带打散为细噪点，得到平滑的变焦虚化感。
            ...
            fixed4 BlurredSample(float2 uv)
            {
                ...
                for (int i = 0; i < BLUR_SAMPLES; i++)
                {
                    ...
                    float w = 1.0 - 0.5 * t;
                    sum += (tex2D(_MainTex, uv + off) + _TextureSampleAdd) * w;
                    total += w;
                }
                return sum / total;
            }
